...
export default function Home(){
 ...
 const [caseId,setCaseId]=useState('owl-pilot'),[step,setStep]=useState(-1),[run,setRun]=useState(0);
 const story=revisedStories.find(item=>item.id===caseId)??revisedStories[0],t=translations[language];
 useEffect(()=>{document.documentElement.lang=language;document.title=`${localeOf(story,language).title} · DetectiveGirl`;try{localStorage.setItem('detective-girl-language',language);}catch{/* Local storage is optional. */}},[language,story]);
 function select(id:string){setCaseId(id);setStep(-1);setRun(value=>value+1);}
 const sceneIndex=step<0?0:localeOf(story,language).steps[Math.min(step,3)].scene;
 const owlList=[owlScenes.corridor,owlScenes.courtyard,owlScenes.library,owlScenes.theatre];
 const scene=story.id.startsWith('owl')?owlList[sceneIndex]:story.id==='forest'?forestScenes[sceneIndex]:{...forestScenes[sceneIndex],image:`backgrounds/${story.id}-${sceneIndex}.jpg`};
